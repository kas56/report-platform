...
--Создаю представление с данными для форммирования квартального отчёта
create view h2020.v_data_quarter_report as
...
join h2020.balance_account bacc
 ...
 and bacc.d_date_balance = (select ldfr.last_day_quarter
                              from h2020.v_last_days_for_reports ldfr
                             where ldfr.year = extract('year' from bacc.d_date_balance)
                               and ldfr.quarter = extract('quarter' from bacc.d_date_balance)
                             limit 1)
join h2020.okv okv
...
group by extract('year' from bacc.d_date_balance),
         extract('quarter' from bacc.d_date_balance),
         org.external_id,
         org.s_org_inn,
         org.s_org_kpp,
...
comment on column h2020.v_data_quarter_report.quarter
  is 'квартал баланса счетов';
