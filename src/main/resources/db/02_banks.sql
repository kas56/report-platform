...
(
  ...
  s_bik              varchar(9) constraint bank_bik check (length(regexp_replace(s_bik, '^[[:digit:]]{9}$', '')) = 0),
  s_swift            varchar(11) constraint bank_swift check ((length(regexp_replace(s_swift, '^([[:digit:]]|[QWERTYUIOPLKJHGFDSAZXCVBNM]){8}$', '')) = 0)
                                                        or (length(regexp_replace(s_swift, '^([[:digit:]]|[QWERTYUIOPLKJHGFDSAZXCVBNM]){11}$', '')) = 0)),
  s_ksnp             varchar(32) constraint bank_ksnp check (length(regexp_replace(s_ksnp, '^[[:digit:]]*$', '')) = 0),
  s_namep            varchar(256) not null,
  ...
  s_regn             varchar(20),
  i_status           numeric(1),
  s_bank_inn         varchar(10) constraint bank_inn check (length(regexp_replace(s_bank_inn, '^[[:digit:]]{10}$', '')) = 0),
  s_bank_kpp         varchar(9) constraint bank_kpp check (length(regexp_replace(s_bank_kpp, '^[[:digit:]]{9}$', '')) = 0),
